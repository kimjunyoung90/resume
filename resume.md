...
- 컬럼에 함수 적용 시 인덱스 원본 값과 달라져 인덱스가 있더라도 활용 불가 → **함수가 인덱스 활용을 막고 있는지 인덱스 점검 필요**

**3) 인덱스 분석**
- **인덱스 확인 결과 작성일자 포함된 인덱스 자체가 부재** — 함수 제거만으로는 해결 불가, 복합 인덱스 신설도 함께 필요함을 확인
- 사업자번호는 **특정 복합 인덱스의 4번째(비선행) 위치**였음
- PostgreSQL이 **인덱스 전체를 순회하는 형태로 동작**
- 두 조건을 효율적으로 탐색할 **복합 인덱스 부재 확인**
