...
- 멱등성 보장을 위해 멱등키를 파라미터로 받고 있었으나 기존 구조에는 근본적인 제약이 존재
- 멱등키 NULL 허용, UNIQUE 제약조건이 없어 중복된 멱등키가 이미 테이블에 다수 존재
- 멱등키 중복 체크 로직이 SELECT EXISTS 기반이라 동시성 환경에서 보장 불가
- 운영 DB의 기존 대량 데이터·NULL 값·중복 값 때문에 직접 제약조건을 추가하기 어려운 환경
