...
- **데이터 기반으로 문제를 분석하고 해결합니다.**
    ...
    APM, Thread Dump, JMeter 등 모니터링·분석 도구를 활용해 데이터로 문제의 근본 원인을 파악합니다. jstack으로 BLOCKED 스레드를 포착하여 synchronized 제거로 latency를 24% 개선했고, 부하 테스트로 개선 효과를 정량적으로 검증했습니다.

- **업무 자동화를 통해 조직 생산성을 높입니다.**

    Confluence Wiki에 API 명세를 수작업으로 작성하던 과정을 **AI와 MCP로 자동화**하고, 적용 및 연동 방법을 정리한 가이드 문서를 작성해 공유했습니다. 또한 다국어 서비스 제공을 위한 반복작업을 자동화하여 **5분 이내로 단축**했습니다.
