...
#### 문제
- 의료마이데이터 서비스는 고객의 진료내역·검진 결과·의료 기록을 사내 데이터 수집 시스템에서 조회해 제공하는 구조
- 데이터 수집 시스템에서 지연 또는 간헐적 장애가 발생하면 의료마이데이터 전체 서비스의 응답이 지연되고, 톰캣 워커 스레드가 고갈되눈 문제 우려
...
- 데이터 수집 저장 로직이 언론사별로 중복되어 있어 신규 제휴 확장 시 개발 공수가 증가
