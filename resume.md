...
- **비효율적인 업무를 개선하여 조직 전체 생산성에 기여합니다.**
	...
	다국어로 서비스 제공하기 위한 반복작업을 자동화하여 5분 이내로 단축했고, 오픈소스로 배포해 내부뿐 아니라 외부 개발자들도 빠르게 다국어 서비스를 제공할 수 있도록 기여했습니다.
...
#### 해결 전략
1) N:M 매핑 구조의 데이터베이스 재설계
- `platform_categories` 테이블 신설: 플랫폼(WEHAGO, NAHAGO)별 표준 카테고리 관리
- `categories` 테이블 재정의: 언론사별 원본 카테고리를 서브 카테고리로 활용
- `categories_mapping` 테이블 추가: N:M 관계 구현으로 하나의 언론사 카테고리가 여러 플랫폼 카테고리에 매핑 가능하도록 설계

2) 쿼리 복잡도 증가 문제 해결
- DB 구조 변경으로 기존 단순 쿼리 → JOIN으로 복잡도 증가
- MyBatis SQL 스니펫을 활용하여 공통 JOIN 로직 재사용
...
#### 성과
- N:M 매핑 구조로 **DB 구조 변경 없이 매핑 데이터만 추가**하여 비즈워치 외 2개 언론사 확장
- 전략 패턴 적용으로 신규 언론사 추가 시 개발 공수 절감
- API 버전 관리로 **기존 클라이언트 영향 없이** 신규 기능 배포
...
- 커뮤니티 MCP를 찾아 안정성 검증 및 연동 테스트
- AI가 코드를 탐색해 명세서를 자동으로 작성하는 동작 확인
- 팀원 및 부서 개발자들이 바로 사용할 수 있도록 연동 가이드 문서 및 프롬프트 공유

#### 성과
- 반복적인 API 명세 작성 작업이 자동화되어 낭비되는 비용을 줄임
- 주변 업무의 비효율성을 쉽게 개선할 수 있다는 인식을 남김
...
#### 해결 전략
- babel parser를 활용해 React 컴포넌트를 AST(Abstract Syntax Tree)로 파싱
- JSX Element와 StringLiteral 노드를 순회하며 한국어 패턴 정규식으로 탐지
- 하드코딩된 한국어 문자열을 react-intl의 FormattedMessage 컴포넌트로 자동 변환
- 탐지된 한국어 문자열을 JSON 형태의 메시지 파일에 자동 추가
- 기존 메시지 파일과 병합하여 키 중복 및 누락 방지
- Commander.js로 사용자 친화적인 CLI 인터페이스 구현
