...
#### 문제
- 고객사의 비정상 재요청(중복 클릭·응답 미수신 재시도 등)으로 동일 세금계산서가 중복 발행되는 문제 발생
- 멱등키 파라미터는 존재했으나 NULL 허용, UNIQUE 제약 부재 등 미비점 존재
- 운영 DB에 NULL·중복 멱등키 데이터가 이미 다수 존재해 기존 테이블에 제약조건을 추가하기 어려운 제약상황
- SELECT EXISTS 기반 중복 체크 로직이 존재했으나 동시성 환경에서 신뢰할 수 없는 로직

#### 해결 전략
1) 멱등키 전용 테이블 신규 설계
- 기존 세금계산서 테이블을 건드리지 않고 멱등키만 관리하는 신규 테이블 구축
- NULL·중복 데이터가 포함된 문서 테이블과 분리해 안정적으로 멱등성 보장 구조 적용

2) 고객 단위 복합 Primary Key 적용
- (고객 고유번호, 멱등키) 조합을 PK로 정의해 고객별 멱등키 고유성 확보
- 동일 고객의 동일 멱등키 요청은 PK 충돌로 즉시 차단

3) DB 기반 멱등성 보장 로직 구현
- 발행 요청마다 멱등키 테이블 INSERT를 시도하고 duplicate key 발생 시 중복 요청으로 간주
- PK 충돌시 기발행 응답 반환

4) 멱등키 미전달 고객 호환성 처리
- 멱등키 미전달 고객은 멱등키 로직 우회되도록 분기해 영향도 제거

#### 성과
- 동일 세금계산서 중복 발행 문제를 차단하여 API 신뢰도 향상
- NULL·중복 멱등키가 포함된 기존 데이터 수정 없이도 멱등성 보장
- 기존 고객의 기능 영향도 없이 안전하게 멱등성 보장 체계 도입
...
- 또한, 데이터 수집 저장 로직이 언론사별로 중복되어 있어 신규 제휴 확장 시 개발 공수가 증가
...
- 언론사별 데이터 수집 부분 및 파싱 로직을 전략 패턴을 사용하여 공통 NewsCollector 인터페이스로 통합
- 신규 언론사 추가 시 기존 비즈니스 로직을 수정할 필요 없이 전략 클래스만 추가하면 확장이 가능하도록 설계
