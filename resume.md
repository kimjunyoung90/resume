...
- 기존에는 카테고리를 정의하는 `categories` 이 존재하고, 해당 카테고리를 조회해서 UI에 노출하는 방식, 뉴스 데이터는 어떤 카테고리에 속해있는지 자신이 속한 카테고리 정보를 담고 있었음
- 카테고리를 추가하면 UI에 카테고리가 무분별하게 노출되고 카테고리를 수정하면 저장된 뉴스 데이터의 매핑 정보를 전부 변경해야하는 상황
- 정치·경제·사회·IT·문화 등 플랫폼 전용 카테고리를 정의한 `platform_categories` 테이블 신설
- 언론사별 카테고리(`categories`)를 플랫폼 표준 카테고리에 매핑하는 `category_mapping` 테이블 설계
- 신규 언론사 추가 시 매핑 데이터만 등록하면 확장 가능하도록 설계
- 플랫폼 카테고리에 변경 시에도 매핑 데이터 수정만으로 신문사의 다른 카테고리 뉴스를 조회 가능하도록 설계

2) 캐시 계층 도입
- 카테고리 조회 시점에 `category_mapping`을 참조하여 플랫폼 표준 카테고리 기준으로 `categories` 조회
