...
- **③ Redis 분산 락**
	- 멱등키를 활용한 분산락을 적용하여 동일 멱등키 동시 발행 차단

**3) 멱등키 전용 테이블 적용**
- 운영 영향도 최소화 고려
- Redis 분산 락은 **신규 인프라 구축 필요** + **Redis 장애 시 멱등성 보장 방법의 부재**
- 함수형 인덱스는 500GB 테이블에 인덱스를 생성해야하는 부담 + 기존 다른 테이블에 인덱스 생성 시 장애로 이어진 경험으로 혹시 발생할 장애 우려
- 신규 테이블 방식은 기존 테이블 영향 없이 **즉시 적용 가능**
- INSERT 추가되지만 발행 트래픽이 최대 100tpm 수준이라 비용이 크지 않다고 판단
