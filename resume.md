...
**3) 채택안 결정 및 근거**
- 운영 영향도 최소화를 기준으로 선택
- Redis 분산 락: **신규 인프라 도입 부담** 및 **Redis 장애 시 멱등성 보장 메커니즘 부재**
- 함수형 인덱스: 500GB 테이블에 인덱스 생성 부담, 과거 유사 작업에서 장애 발생 이력 존재
- 신규 테이블 방식: 기존 테이블 영향 없이 **즉시 적용 가능**
- 멱등키 INSERT 1회가 추가되나 발행 트래픽이 최대 100 TPM 수준으로 부하 영향 미미
