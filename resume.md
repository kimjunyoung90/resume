...
- **AI 도구를 활용해 업무 효율성을 높입니다.** 
    ...
    Confluence Wiki에 API 명세를 수작업으로 작성하던 과정을 AI와 MCP로 자동화하고, 적용 및 연동 방법을 정리한 가이드 문서를 작성해 조직 전체가 활용할 수 있도록 공유했습니다.
    
- **반복되는 개발 과정의 비효율을 스스로 개선합니다.**

	클라이언트 애플리케이션에서 다국어 서비스를 제공하기 위해 하드코딩된 문자열을 일일이 메시지로 변환하던 과정을 npm 모듈로 자동화해 기존에 수십 분이 소요되던 작업을 5분 이내로 단축했고, 이를 오픈소스로 배포해 내부뿐 아니라 외부 개발자들도 자동으로 다국어 처리를 수행할 수 있도록 했습니다.
...
#### 문제
- APM에서 특정 API p95 응답시간이 25초까지 상승
...
#### 문제
- 고객사의 비정상 요청(중복 요청·응답 미수신 시 재시도)으로 동일 세금계산서가 중복 발행되는 문제 발생
...
## 업무 생산성 개선 – 다국어 처리 자동화 CLI 개발 (2024)
