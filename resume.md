...
- 데이터 수집 시스템에서 지연 또는 간헐적 장애가 발생하면 의료마이데이터 전체 서비스의 응답이 지연되고, 톰캣 워커 스레드가 고갈되는 문제 우려
...
- 기존 구조는 categories(신문사 카테고리) 데이터를 UI에 직접 노출하는 방식이며, 뉴스 데이터는 자신이 속한 카테고리 코드를 저장하고 있는 형태
