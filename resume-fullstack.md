...
- **AI 도구를 적극 활용해 개발 생산성을 높입니다.**
    ...
    Kibana API를 통해 Elastic APM Agent로 수집된 모니터링 지표를 자연어로 조회하는 [MCP 서버|https://www.npmjs.com/package/elastic-apm-mcp-server]를 개발해, 모니터링 지표 확인 과정을 간소화하고 운영 효율을 높였습니다.
...
- 시스템의 CPU·메모리 확인 결과 CPU 5%, 메모리 30%로 정상 범위로 내부 로직의 비효율은 아니라고 판단
...
- 기존 Index Scan 사용된 Index는 사업자번호가 **복합 인덱스의 비선행(3번째) 컬럼**
...
- 보상 트랜잭션(포인트 차감 취소) 실패 시 데이터 정합성을 회복할 메커니즘이 부재
...
**① 응집도 개선**
- 거대 단일 서비스(21,430줄)를 **11개 도메인 서비스로 분리**
...
## 전자세금계산서 프론트엔드 개발 및 유지보수 (2021.09 ~ 현재)

- **6종 전자문서**(세금계산서·위수탁·거래명세서·영수증·입금표·전자청구서)의 발행·조회 화면 개발·유지보수
- 신규 기능 개발과 함께, **노후한 레거시 프론트 코드를 구조적으로 개선하는 작업을 병행**
- 주요 작업
  - **기능 개발**: 외상매출금 입금확인 신규 서비스, 발행방법 공통 설정 기능
  - **기능 개선**: 발행 유효성 검증, 거래처 휴폐업 검증, 인증서 모듈, 다국어 등 이슈 **189건** 처리
  - **레거시 개선**: jQuery 제거, 불필요한 props 전달 제거, 중복 로직 공통화
  - **성능 개선**: code splitting으로 초기 로딩 개선, 렌더링 최적화
